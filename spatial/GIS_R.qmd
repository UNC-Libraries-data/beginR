...
title: "GIS in R"
author: 
  name: UNC University Library, Library Data Services
  url: https://library.unc.edu/data/
execute:
  echo: true
format: 
  html:
...
If you'd like a refresher on R, we suggest the [R For Data Science](https://r4ds.hadley.nz/) free e-book and the lessons from our [beginR workshops](https://tarheels.live/beginr/lessons/), including [setup instructions for R and R Studio](../week1_Introduction/R_OpenLabs_Introduction.html#setup-r-r-studio).
...
`install.packages(c('tidyverse','scales','sf','tidycensus','tigris','arcgislayers','ggmap','ggspatial','basemaps','tmap'))`
...
#Thematic mapping and formatting
library(ggmap)
library(ggspatial)
library(basemaps)
library(scales)
...
The `sf` packages will be our main spatial processing tool for today.
...
```{r}
#| message: false
nc_tracts<- get_acs(geography = "tract", state = "NC", variables = c("B01001_001"),
...
```{r}
nc_tracts |> slice_sample(n=5)
```

With `slice_sample` we get random rows from anywhere in the dataset - your results probably look different. This looks like a normal R dataframe except for the additional spatial information at the top and the geometry column. In this case, each entry in the geometry column is a set of points defining a (multi)polygon.

[Read more about the geometry types available in `sf`](https://r-spatial.github.io/sf/articles/sf1.html#simple-feature-geometry-types)

```{r}
# First 10 points of the first feature
as.matrix(nc_tracts$geometry[[1]])[1:10,]
```
...
Which tracts are over 15,000 population? We can use `dplyr`'s `filter` as usual, but we'll add an `sf` function: `st_drop_geometry` to drop the spatial elements to create a normal dataframe. This will make our output a little more compact (try it without st_drop_geometry!). Removing the geometry can also speed up other operations - it's worth considering whenever you're doing more complicated data transformations. You can always use a key variable to rejoin the geometries later on.

Most functions in the `sf` package start with `st_` - we will see many more throughout the workshop.
...
[Download nc_public_housing.csv](data/nc_public_housing.csv)
...
There are a number of ways to get basemaps in R, but for today we'll use the `basemaps` package. Most map services now require an API key to use. These keys are often free, but require sign up to get access and may involve some level of rate limiting. Maps from Open Street Map and ESRI are currently freely available without an API key. You can check the maps available with `get_maptypes("esri")` or `get_maptypes("osm")`.

`st_bbox` converts our set of points into a rectangle to form our plot area. We'll use `theme_nothing` from `ggmap` to remove all axes and other plot elements.

```{r}
#| warning: false
#basemaps package map services use Web Mercator (ESPG: 3857)
basemap_ggplot(ext = st_bbox(housing), map_service = "osm", map_type = "streets", verbose=FALSE)+
...
basemaps::basemap_ggplot(ext = st_bbox(st_buffer(nc_boundary, 
                                                 dist = 100000)),
                          map_service = "osm", map_type = "streets")+ #distance in meters for geographic coordinates
  geom_sf(data = housing)+
...
Here we can add some more polish to the plot: - add colors for Urban vs Rural and label appropriately, update labels. - switch to the "world_dark_gray_base" basemap from ESRI (reminder: see `get_maptypes(map_service = "esri")` for a full list of options). - add a scale and north arrow for context - various color and line tweaks for aesthetics
...
We've already seen a simple spatial operation in `st_buffer` to create a buffered polygon expanding on an existing geometry. There are [many other operations](https://github.com/rstudio/cheatsheets/blob/main/sf.pdf) - we'll focus on two major ideas: spatial filtering and spatial joins.
...
Note that we filter with the housing locations not the NC boundary. There are several CSAs that touch NC and would be captured if we used that as a filter:

```{r}
nc_csas_border <- us_csas |> 
  st_filter(nc_boundary, .predicate = st_intersects)
nrow(nc_csas)
nrow(nc_csas_border)
```

However, we could work around this by using the centroids of the CSAs with `st_centroid`.

```{r}
nc_csas_centroids <- st_centroid(us_csas) |> 
  st_filter(nc_boundary, .predicate = st_intersects)
nrow(nc_csas_centroids)
```

The only downside is this leaves us with centroid geometries instead of polygons.

```{r}
ggplot()+
  geom_sf(data = nc_csas_border, fill = "red", alpha = 0.5, color=NA)+
  geom_sf(data=nc_csas, color=NA)+
  geom_sf(data=nc_csas_centroids)+
  geom_sf(data = nc_boundary, fill=NA)+
  theme_minimal()
```

We'll work with our original `nc_csas`. Now that we've limited our CSAs, let's take a look at what made the cut.
...
[`tmap`](https://r-tmap.github.io/tmap/) is another popular package for making thematic maps.  `tmap` uses tile map services like those we pulled from the `basemaps` service.  We'll again focus on Open Street Maps and ESRI maps. You can check the list of available maps with `tmap_providers()`.

`tmap` objects include pop ups to display the data associated with the plotted shapes or points.  We can disable these with `popup=FALSE` as below.

```{r}
#| fig-width: 12
#| fig-height: 5
tm_basemap("Esri.WorldTopoMap", alpha = 1)+
  tm_shape(nc_csas)+
  tm_polygons(fill="NAME", fill_alpha = 0.35,
              fill.legend = tm_legend(
                position = tm_pos_out("right", "center", pos.h = "center")
              ))+
  tm_shape(housing)+
  tm_dots(popup=FALSE)+
  tm_compass()+
  ...
  tm_layout(frame = FALSE, 
            inner.margins = c(0.1,0.1,0,0)) #add space inside the plot
```

By adding `tmap_mode("view")` before our code, we can make an interactive map.  

Let's also use some of our `tidyverse` tools to subset our `housing` dataframe so we can leave the pop ups on.
...
tm_basemap("Esri.WorldTopoMap")+
  tm_shape(nc_csas)+
  tm_polygons(fill="NAME", fill_alpha = 0.25, col = "grey70",
              fill.legend = tm_legend(
                position = tm_pos_out("right", "center", pos.h = "center")
              ))+
  tm_legend_hide()+
  tm_shape(select(housing,PROJECT_NAME,TOTAL_UNITS,TOTAL_OCCUPIED,PCT_OCCUPIED))+
  tm_dots()+
  tm_compass()+
  tm_scalebar()+
  tm_layout(frame = FALSE, 
...
[`leaflet`](https://rstudio.github.io/leaflet/articles/leaflet.html) is the R package for creating interactive maps with the [Leaflet](https://leafletjs.com/) JavaScript library.
...
## Learn more: CRAN Task View

To learn more about other R packages around spatial data processing and analysis, consult the [CRAN Task View: Analysis of Spatial Data](https://cran.r-project.org/web/views/Spatial.html).

For example, the `spdep` and `spatialreg` packages provide functions for weights-matrix based methods like Moran's I and Spatial Autoregressive Models.
